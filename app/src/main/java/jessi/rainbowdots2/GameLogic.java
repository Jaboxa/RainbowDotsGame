...
public class  GameLogic extends View  implements View.OnTouchListener{
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < pointListEasy.size(); i++) {
            paintEasy.setARGB(250, colorArrayEasy[i][1], colorArrayEasy[i][2], colorArrayEasy[i][3]); //opaque rainbow
        ...
        canvas.drawText("You have 20s!",myWidth/10,myHeight*2/3+100,paintEasy);
    }
    public Bitmap takeScreenShot(GameLogic gameLogic) {

        gameLogic.setDrawingCacheEnabled(true);
        gameLogic.setDrawingCacheQuality(View.DRAWING_CACHE_QUALITY_LOW);
        gameLogic.buildDrawingCache();

        if(gameLogic.getDrawingCache() == null) return null;

        snapshot = Bitmap.createBitmap(gameLogic.getDrawingCache());
        gameLogic.setDrawingCacheEnabled(false);
        gameLogic.destroyDrawingCache();
    ...
    public boolean gameLost() {
        ...
        for (int i = 1; i <= (myHeight/2); i++) {
            for (int j = 1; j <= (myWidth*3/4) ; j++) {
                int pixel = snapshot.getPixel(i, j);
